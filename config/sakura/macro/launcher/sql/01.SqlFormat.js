...
var bSQLFMTCMD = true;								// java コマンドを使用するか？
var SQLFMTCMD = "sqlbeautify.jar";		// 実行する jar ファイル名
var JAVA_FOLDER_PATH = "C:\\Program Files\\Java";	// java のインストールパス

/*
 * 文字種別
 */
var CNS_NORMAL=0; // 通常種別
var CNS_CHAR=1; // SQL中の文字列を表す種別「' … '」
var CNS_DC=2; // SQL中の文字列を表す種別「" … "」
var CNS_COMMENT_1=3; // SQL中のコメントを表す種別「/* … */」
var CNS_COMMENT_2=4; // SQL中のコメントを表す種別「-- … 」
var CNS_HINT=5; // SQL中のヒント句を表す種別「/*+ … */ 」
var CNS_RM=-1; // 除去対象となる文字を表す種別
...
// 整形SQLを取得
function getFormatSql( text ){
	var ForReading = 1;
	var regex = /([ \t]*(IN|DECODE|SIGN|MOD|ROUND|TRUNC|MAX|MIN|AVG|SUM|SUBSTR|TO_CHAR|EXTRACT|EXTRACTVALUE))[ \t]*\(([^\)]+)\)/ig;
	var sqltext = "";
	var fso = new ActiveXObject("Scripting.FileSystemObject");
	var cmdpath = getCommandFolderPath() + SQLFMTCMD;
	
	if ( bSQLFMTCMD && fso.FolderExists(JAVA_FOLDER_PATH) && fso.FileExists(cmdpath) ) {
		
		var tempFilePath = fso.GetSpecialFolder(2)+"\\"+fso.gettempname();
		
		//TraceOut(text.replace(/^\s+[\r\n]+/gm,''));
		var l = text.replace(/[\r\n]+\s+[\r\n]+/gm,'').split(";");
		var len = l.length;
		for (var i=0; i<len ;i++){
			//TraceOut(i+":"+l[i]);
			...
			var stream = fso.CreateTextFile(tempFilePath, true);
			//stream.Write( text );
			stream.Write( l[i] );
			stream.Close();
			
			new ActiveXObject("WScript.Shell").Run(
				'java -jar "' + cmdpath + '" "' + tempFilePath + '" -o', 0, true);
			
			stream = fso.OpenTextFile(tempFilePath, ForReading);
			if (stream.AtEndOfStream) sqltext += "";
			else sqltext += stream.ReadAll();
			stream.Close();
			
			fso.DeleteFile(tempFilePath);
		}
		sqltext = sqltext.replace(/' '/g,"''");	// ''->' ' に直されてしまうため
		sqltext = sqltext.replace(/([^ \t]) \(/mg, "$1(");	//))
		sqltext = sqltext.replace( regex, function($0, $1, $2, $3) { return $1+"("+$3.replace(/[\r\n \t]+/g,' ')+")"; });
		return sqltext;
	} else {
		return fixSQL( text );
	}
...
function getCommandFolderPath(){
	return new String( Editor.ExpandParameter("$S") ).replace(/[^\\]+$/,"")+"bin\\";
}
