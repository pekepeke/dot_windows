...
o.push(m+'<br>*/</pre>');}
o.push('</div>');}
return o.join('');};itself.edition='2009-06-01';return itself;}());
//if(!JSLINT(WScript.StdIn.ReadAll(),{passfail:true})){var e=JSLINT.errors[0];WScript.StdErr.WriteLine('Lint at line '+(e.line+1)+' character '+(e.character+1)+': '+e.reason);WScript.StdErr.WriteLine((e.evidence||'').replace(/^\s*(\S*(\s+\S+)*)\s*$/,"$1"));WScript.Quit(1);}}());
(function(){
	var e = Editor;
	e.MoveHistSet();
	if (e.IsTextSelected == 0) e.SelectAll();
	var text = e.GetSelectedString(0).replace(/\r\n/g,"\n").replace(/\r/g,"\n");	// â¸çsÇ\nÇ≈ìùàÍ
	if (!JSLINT(text, {passfail:true})) {
		var er = JSLINT.errors[0];
		e.TraceOut((er.evidence||'').replace(/^\s*(\S*(\s+\S+)*)\s*$/,"$1"));
//		for (var i=0, l=JSLINT.errors.length; i<l ; i++){
//			var er = JSLINT.errors[i];
//			e.TraceOut((er.evidence||'').replace(/^\s*(\S*(\s+\S+)*)\s*$/,"$1"));
//		}
	}
	e.MoveHistPrev();
